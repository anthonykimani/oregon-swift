...
export function CourierDeliveryContent({
  id,
  token,
  courierName,
  onStatusUpdated,
  ...
  const load = useCallback(() => {
    if (!token) return;
    api<Delivery>(`/courier/deliveries/${id}`, { token })
      .then((res) => {
  ...
  async function handleStatusUpdate(status: string) {
    ...
    setUpdating(true);
    setError("");
    const res = await api(`/courier/deliveries/${delivery.id}/status`, {
      method: "PATCH",
      token,
  ...
  if (!delivery) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
        ...
        <p className="text-sm text-[#8094A7] font-inter">
  ...
  const bookedDate = new Date(delivery.createdAt).toLocaleDateString("en-US", {
    month: "long",
    day: "numeric",
    year: "numeric",
  });
  const isTerminal = terminalStatuses.includes(delivery.status);
  ...
  async function messageAboutDelivery() {
    ...
    try {
      ...
      if ((res.status === 200 || res.status === 201) && res.data?.conversation?.id) {
        router.push(`/courier/messages?thread=${encodeURIComponent(res.data.conversation.id)}`);
      }
    } finally {
      setMessaging(false);
    }
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="sticky top-0 z-10 -mx-4 -mt-4 flex items-start justify-between gap-4 border-b border-[#DCE2D9] bg-[#F3F5F1]/95 px-4 py-5 pr-20 backdrop-blur sm:-mx-6 sm:-mt-6 sm:px-6 sm:py-6 sm:pr-20">
        <div className="min-w-0">
          <p className="mb-1 text-xs font-bold uppercase tracking-[0.14em] text-forest-600">Active job</p>
      ...
      {!isTerminal && (
        <Card className="border-[#DCE2D9] rounded-2xl shadow-none bg-white">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base text-[#052D50]">
              <Truck className="h-5 w-5 text-[#173420]" />
              Update Status
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 gap-2">
              {statusActions.map((action) => {
                ...
                const disabled = updating || delivery.status === action.status;
                return (
                  <button
                    key={action.status}
                    type="button"
            ...
            {error && (
              <div className="mt-3 bg-[#FCDEE0] text-[#C0392B] text-sm rounded-lg px-4 py-3">
                {error}
              </div>
            )}
          </CardContent>
...
interface CourierDeliverySheetProps {
  open: boolean;
  id: string | null;
  token?: string;
  courierName?: string | null;
...
export function CourierDeliverySheet({
  open,
  id,
  token,
  courierName,
